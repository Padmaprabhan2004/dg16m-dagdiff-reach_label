...
class DexNet(object):
    ...
    def _single_obj_grasps(dataset, obj, gripper, config, stable_pose_id=None, single_arm=False, target_num_grasps=100, num_workers=8):
        ...
        scale, grasps = sampler.generate_grasps(
            obj, check_collisions=True,
            max_iter=10,
            grasp_gen_mult=1.25, config=config, target_num_grasps=target_num_grasps, 
            single_arm=single_arm, num_workers=num_workers)#collision check and distance pruning is done here
        return scale, grasps
        # scale, f, d, t, grasps = sampler.generate_grasps(
        #     obj, check_collisions=True,
        #     max_iter=config['max_grasp_sampling_iters'],
        #     grasp_gen_mult=config['grasp_gen_mult'], config=config, target_num_grasps=target_num_grasps, 
