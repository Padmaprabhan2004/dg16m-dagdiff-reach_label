...
class GraspSampler:
    ...
    def generate_grasps(self, graspable, target_num_grasps=None, grasp_gen_mult=1, max_iter=3,
        ...
        while num_grasps_remaining > 0 and k <= max_iter:
            ...
            # COVERAGE REJECTION: prune grasps by distance->dg16m specific
            pruned_grasps = []
            ...
            #! remove grasps that are too close  #DISTANCE BASED PRUNING STEP
            ...
            # return scale, f_list, d_list, t_list, final_grasps
            if not len(dual_grasp):
                k += 1
                continue
        ...
        if len(final_grasps) == 0:
            print("no dual grasp, skip da object")
            return scale, np.empty((0,2), dtype=object)

        print(f"Found {len(grasps)} single-arm grasps.")
        print(f"Found {len(final_grasps)} dual-arm grasps.")

        return scale, final_grasps
...
class MeshAntipodalGraspSampler(GraspSampler):
    ...
    def sample_hierachical_grasps(self, graspable, num_grasps, gripper=None,vis=False, num_workers=8):
        ...
        #     grasp_list.append(submesh_grasps)
        #     print(f'Submesh {i} has {len(submesh_grasps)} grasps')
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=min(num_workers, 8)) as executor:
            grasp_list = list(executor.map(self.multi_grasp, 
                                           submesh_indices, 
                                           submesh_list, 
                                           [graspable]*len(submesh_list), 
                                           [subgrasp]*len(submesh_list), 
                                           [gripper]*len(submesh_list)))
            

        # grasp_list_flat = []
        
        # for grasp in grasp_list:
        #     grasp_list_flat.extend(grasp)
