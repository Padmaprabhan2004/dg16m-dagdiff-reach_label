...
class TimeLatentFeatureEncoder(nn.Module):
    def __init__(
        ...
        print(f"Number of layers in TimeLatentFeatureEncoder: {self.num_layers}")
...
class TimeLatentFeatureEncoderGraspConditioned(nn.Module):
    def __init__(
        self,
        latent_size,
        dims,
        ...
        super(TimeLatentFeatureEncoderGraspConditioned, self).__init__()
        ...
        self.grasp_condition_dim = n_points * out_dim
        ...
        if self.recurrent:
            dims[0] = 768
            dims[1] = 768
            dims = [latent_size + self.grasp_condition_dim + enc_dim + in_dim] + dims + [out_dim]
        else:
            dims = [latent_size + enc_dim + in_dim] + dims + [out_dim]
    ...
    # input: N x (L+3)
    def forward(self, input, timesteps, latent_vecs=None, grasp_condition=None):
        # print(f"input shapeeeeeeee: {input.shape}")
        ...
        if (latent_vecs is not None):
            latent_vecs = F.dropout(latent_vecs, p=0.2, training=self.training)
            if self.recurrent:
                if grasp_condition is None:
                    grasp_condition = torch.zeros(*input.shape[:-1], self.grasp_condition_dim).cuda()
                else:
                    if len(latent_vecs.shape) == 3 and len(grasp_condition.shape) == 2: # B x n_points x latent_dim
                        grasp_condition = grasp_condition.unsqueeze(1).repeat(1, self.n_points, 1)
                # print(f'grasp_condition.shape: {grasp_condition.shape}')
                x = torch.cat([grasp_condition, latent_vecs, xyz, input], -1)
                # print(f'all concatenated x.shape: {x.shape}')
                # print("=====================================================================================")
            else:
                x = torch.cat([latent_vecs, xyz, input], -1)
        else:
