...
def create_gripper_marker_da2(color=[0, 0, 255], tube_radius=0.002, sections=6, scale = 1.):
    """Create a 3D mesh visualizing a parallel yaw gripper. It consists of four cylinders.
    ...
    cfl = trimesh.creation.cylinder(
        ...
        sections=sections,
        segment=[
            [4.10000000e-02 * scale, -7.27595772e-12 * scale, 0],
            [4.10000000e-02 * scale, -7.27595772e-12 * scale, 0.067500 * scale],
        ],
    )
    cfr = trimesh.creation.cylinder(
        radius=tube_radius * scale,
        sections=sections,
        segment=[
            [-4.100000e-02 * scale, -7.27595772e-12 * scale, 0 * scale],
            [-4.100000e-02 * scale, -7.27595772e-12 * scale, 0.067500 * scale],
        ],
    )
    cb1 = trimesh.creation.cylinder(
        radius=tube_radius * scale, sections=sections, segment=[[0, 0, -0.067500/2 * scale], [0, 0, 0]]
    )
    cb2 = trimesh.creation.cylinder(
        radius=tube_radius * scale,
        sections=sections,
        segment=[[-0.085/2 * scale, 0, 0], [0.085/2 * scale, 0, 0]],
    )
...
def visualize_grasps(Hs, scale=1., p_cloud=None, energies=None, colors=None, mesh=None, show=False, save=True, da2=False):
    ...
    for k in range(Hs.shape[0]):
        ...
        if da2:
            grips.append(
                create_gripper_marker_da2(color=c_vis, scale=scale).apply_transform(H)
            )
        else:
            grips.append(
    ...
    if show:
        ...
        scene.export('output_mesh.obj')
        import open3d as o3d
        # mesh = o3d.io.read_triangle_mesh('output_mesh.obj')
