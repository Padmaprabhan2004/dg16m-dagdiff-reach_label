...
class ConvGraspDiffusionFields(GraspDiffusionFields):
    ...
    def __init__(self, vision_encoder, vision_decoder, geometry_encoder, 
                 ...
                 use_attention=False, classifier=None, collision_predictor=None, reachability_classifier=None):
        ...
        self.reachability_classifier= reachability_classifier
    def set_latent(self, O, batch = 1):#triplane projections
        self.z = self.vision_encoder(O.squeeze(1))

    def get_latent(self, local_context=None):
        out = self.vision_decoder(local_context, self.z)#bilinear interpolator
        return out
    
    def compute_sdf(self, x):#feature encoder outputs
    ...
    def part_forward(self,p,k,batch=1, dual=False):
        ...
        e = self.decoder(psi_flatten_energy) # decoder is energy mlp, the score matching energy
        
        if self.classifier is not None:
            self.pred_label = self.classifier(psi_flatten_energy)
        if self.reachability_classifier is not None:
            self.reach_label = self.reachability_classifier(psi_flatten_energy)
        return e
