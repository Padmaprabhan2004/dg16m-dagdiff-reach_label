...
class VNNLocalPoolPointnet(nn.Module):
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, scatter_type='max', 
                 ...
                 k=20, device='cpu',dex=False, unet_depth=4):
        super().__init__()
        self.c_dim = c_dim
        ...
        if self.dex:
            self.fc_pool = VNLinear(hidden_dim, c_dim)
        ...
        if unet:
            self.unet = UNet(self.c_dim_flattened, in_channels=self.c_dim_flattened, depth=unet_depth)
        else:
            self.unet = None
    ...
    def forward(self, p):
        ...
        for block in self.blocks[1:]: #this only considers 1:3
        ...
        if self.dex:
            global_feat = self.fc_pool(pooled).reshape(batch_size,-1)
