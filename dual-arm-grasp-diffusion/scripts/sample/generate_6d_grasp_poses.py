...
def parse_args():
    ...
    p.add_argument('--input', type=str, required=False, default="")
...
def get_approximated_grasp_diffusion_field(p, args, device='cpu'):
    ...
    generator = Grasp_AnnealedLD(model, batch=batch, T=140, T_fit=50, k_steps=2, device=device)
...
def sample_pointcloud(input_path=None):
    ...
    print(f"Max point of P: {np.max(P, axis=0)}")
...
if __name__ == '__main__':
    ...
    # sdf = model.compute_sdf(to_torch(P[None,...], device))
    # print(sdf.shape)
    # torch.save(sdf.cpu(), './sdf.pt')
    # torch.save(P, './pointcloud.pt')
    # print(generator)
    # print(model)
    ...
    mask = e < -85 # emperically set threshold, changes with different ckpt.
    # print(torch.sort(e).values)
    ...
    mesh.export('./only_output_mesh.obj')
    torch.save(H.detach().cpu(), './output_single_H.pt')
    torch.save(e[mask], './output_single_energy.pt')
